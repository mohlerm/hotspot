...
#JAVAARGS_CREATE = ['-agentlib:hprof=cpu=times', '-XX:+UnlockDiagnosticVMOptions', '-XX:+PrintCompilation', '-XX:-UseOnStackReplacement', '-XX:+UnlockExperimentalVMOptions', '-XX:+TraceDeoptimization', '-Xbatch', '-XX:CompileCommandFile=createCommands.txt']
...
JAVAARGS_CREATE = ['-XX:+DumpProfiles']
...
try:
    ...
    while(counter < NROFRUNS):
        ...
        m = re.search(r"\d\s(\d*\.\d*\%)\s\d*\.\d*\%\s*\d*\s\d*\sLongNoBranches",file)
