...
array = ['compress', 'crypto.aes', 'crypto.rsa', 'crypto.signverify', 'derby', 'mpegaudio', 'scimark.fft.large', 'scimark.lu.large', 'scimark.sor.large', 'scimark.sparse.large', 'scimark.fft.small', 'scimark.lu.small', 'scimark.sor.small', 'scimark.sparse.small', 'scimark.monte_carlo', 'serial', 'sunflow', 'xml.transform']
oldsum = 0
for bench in array:
    for i in range(1,61):
        if i<10:
    ...
    print('sh run-benchmark.sh create '+ bench + ' | tee singlebench/'+ bench + '.create')
    for i in range(1,61):
        if i<10:
