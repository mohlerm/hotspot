I continue with presenting two very simple example methods that illustrate the motivation from using cached profiles.
...
In a next execution I use these profiles and achieve a significantly lower time spend executing the cached method as one can see in Figure \ref{f:nocompile}.
...
\end{figure}
\\\\
Enabling OSR vanishes the difference between with and without using cached profiles.
This happens because HotSpot quickly realizes the hotness of the method and the simplicity of the method allows the JIT compiler to produce optimal code already. The interpreted version gets replaced on the stack by the compiled version during he first method invocation. 
The optimality of the compiled code is is confirmed by the fact that no deoptimizations occur.
...
\section{Example 2}
\label{s:ex2}
However, OSR is one of the core features of HotSpot to improve startup performance of a JVM and disabling that does not give us any practical results. I came up with a second simple example sketched in Listing \ref{l:manydeopts}, which is slightly more complex but demonstrates the influence of cached profiles without disabling any HotSpot functionalities, 
\\\\
...
The reasons are deoptimizations, that occur for example due to uncommon branch paths are taken or yet unused methods are invoked.
In the past their clients used techniques to warm up the JVM, for example doing fake trades prior to market opening. However this does not solve the problem sufficiently, since the JVM optimizes for these fake trades and still runs into deoptimizations once actual trades are meant to happen. That is, because the code includes methods or specific code snippets that differentiate between the fake and the real trades.
\\\\
